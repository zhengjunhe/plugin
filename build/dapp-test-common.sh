...
dplatform_SignAndSendTxWait() {
    # txHex="$1" priKey="$2" MAIN_HTTP="$3" FUNCNAME="$4"
    req='{"method":"Dplatform.DecodeRawTransaction","params":[{"txHex":"'"$1"'"}]}'
    dplatform_Http "$req" "$3" '(.result.txs[0].execer != "") and (.result.txs[0].execer != null)' "$4"
    dplatform_SignAndSendTx "$1" "$2" "$3"
    dplatform_BlockWait 1 "$3"
}

dplatform_BlockWait() {
    local MAIN_HTTP=$2
    local req='"method":"Dplatform.GetLastHeader","params":[]'
...
dplatform_QueryTx() {
    ...
    local txhash="$1"
    local req='"method":"Dplatform.QueryTransaction","params":[{"hash":"'"$txhash"'"}]'
...
dplatform_SendTx() {
    ...
    if [ "$err" == null ]; then
        dplatform_QueryTx "$txhash" "$MAIN_HTTP"
    else
...
dplatform_SendToAddress() {
    ...
    local req='"method":"Dplatform.SendToAddress", "params":[{"from":"'"$from"'","to":"'"$to"'", "amount":'"$amount"', "note":"test\n"}]'
    resp=$(curl -ksd "{$req}" "${MAIN_HTTP}")
    ...
    echo "hash"
    dplatform_QueryTx "$hash" "$MAIN_HTTP"
}
...
dplatform_ImportPrivkey() {
    ...
    local req='"method":"Dplatform.ImportPrivkey", "params":[{"privkey":"'"$pri"'", "label":"'"$label"'"}]'
    resp=$(curl -ksd "{$req}" "$MAIN_HTTP")
...
dplatform_SignAndSendTx() {
    ...
    local req='"method":"Dplatform.SignRawTx","params":[{"privkey":"'"$priKey"'","txHex":"'"$txHex"'","expire":"'"$expire"'"}]'
    signedTx=$(curl -ksd "{$req}" "${MAIN_HTTP}" | jq -r ".result")

    if [ "$signedTx" != null ]; then
        dplatform_SendTx "$signedTx" "${MAIN_HTTP}"
    else
        echo "signedTx null error"
    fi
}

dplatform_QueryBalance() {
    local addr=$1
    local MAIN_HTTP=$2
    req='"method":"Dplatform.GetAllExecBalance","params":[{"addr":"'"${addr}"'"}]'
    #echo "#request: $req"
    resp=$(curl -ksd "{$req}" "${MAIN_HTTP}")
    echo "#response: $resp"
...
dplatform_QueryExecBalance() {
    local addr=$1
    local exec=$2
    local MAIN_HTTP=$3

    req='{"method":"Dplatform.GetBalance", "params":[{"addresses" : ["'"${addr}"'"], "execer" : "'"${exec}"'"}]}'
    resp=$(curl -ksd "$req" "${MAIN_HTTP}")
    echo "#response: $resp"
...
dplatform_GetAccounts() {
    local MAIN_HTTP=$1
    resp=$(curl -ksd '{"jsonrpc":"2.0","id":2,"method":"Dplatform.GetAccounts","params":[{}]}' -H 'content-type:text/plain;' "${MAIN_HTTP}")
    echo "$resp"
}

dplatform_LastBlockhash() {
    local MAIN_HTTP=$1
    result=$(curl -ksd '{"method":"Dplatform.GetLastHeader","params":[{}]}' -H 'content-type:text/plain;' "${MAIN_HTTP}" | jq -r ".result.hash")
    LAST_BLOCK_HASH=$result
    echo -e "######\\n  last blockhash is $LAST_BLOCK_HASH  \\n######"
}

dplatform_LastBlockHeight() {
    local MAIN_HTTP=$1
    result=$(curl -ksd '{"method":"Dplatform.GetLastHeader","params":[{}]}' -H 'content-type:text/plain;' "${MAIN_HTTP}" | jq -r ".result.height")
    LAST_BLOCK_HEIGHT=$result
    echo -e "######\\n  last blockheight is $LAST_BLOCK_HEIGHT \\n######"
}

dplatform_applyCoins() {
    ...
    if [ "$count" -gt 15000000000 ]; then
        echo "dplatform_getMainCoins wrong coins count,should less than 150 00000000"
        exit 1
    fi
...
dplatform_RpcTestBegin() {
    echo -e "${GRE}====== $1 Rpc Test Begin ===========${NOC}"
}

dplatform_RpcTestRst() {
    if [ -n "$2" ]; then
