...
// GetNonce 目前dplatform中没有保留账户的nonce信息，这里临时添加到合约账户中；
// 所以，目前只有合约对象有nonce值
...
// 因为dplatform的限制，在执行器中转账只能在以下几个方向进行：
// A账户的X合约 <-> B账户的X合约；
...
// dplatform转账限制请参考方法 Transfer2Contract ；
...
// GetConfig 获取系统配置
func (mdb *MemoryStateDB) GetConfig() *types.DplatformConfig {
