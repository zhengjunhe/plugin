...
function LockTestChain33ToEthAssets() {
    ...
    # chain33 lock bty
    hash=$(${Chain33Cli} evm call -f 1 -a 1 -c "${chain33DeployAddr}" -e "${chain33BridgeBank}" -p "lock(${ethDeployAddr}, ${chain33BtyTokenAddr}, 100000000)")
...
function BurnTestChain33ToEthAssets() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    # eth burn
    result=$(${CLIA} ethereum burn -m 1 -k "${ethDeployKey}" -r "${chain33DeployAddr}" -t "${ethereumBtyTokenAddr}" ) #--node_addr https://ropsten.infura.io/v3/9e83f296716142ffbaeaafc05790f26c)
    cli_ret "${result}" "burn"
    echo -e "${GRE}=========== $FUNCNAME end ===========${NOC}"
}
...
function LockTestETH2Chain33Assets() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    # ETH 这端 lock 11个
    result=$(${CLIA} ethereum lock -m 2 -k "${ethDeployKey}" -r "${chain33ReceiverAddr}")
    cli_ret "${result}" "lock"
    echo -e "${GRE}=========== $FUNCNAME end ===========${NOC}"
}
...
function BurnTestETH2Chain33Assets() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    result=$(${CLIA} chain33 burn -m 2 -k "${chain33ReceiverAddrKey}" -r "${ethDeployAddr}" -t "${chain33EthTokenAddr}")
    cli_ret "${result}" "burn"
    echo -e "${GRE}=========== $FUNCNAME end ===========${NOC}"
}
...
function LockTestETH2Chain33Ycc() {
    echo -e "${GRE}=========== $FUNCNAME begin ===========${NOC}"
    # ETH 这端 lock 7个 YCC
    result=$(${CLIA} ethereum lock -m 3 -k "${ethDeployKey}" -r "${chain33ReceiverAddr}" -t "${ethereumYccTokenAddr}")
    cli_ret "${result}" "lock"
    echo -e "${GRE}=========== $FUNCNAME end ===========${NOC}"
}
...
function BurnTestETH2Chain33Ycc() {
    ...
    result=$(${CLIA} chain33 burn -m 3 -k "${chain33ReceiverAddrKey}" -r "${ethDeployAddr}" -t "${chain33YccTokenAddr}")
    cli_ret "${result}" "burn"
    echo -e "${GRE}=========== $FUNCNAME end ===========${NOC}"
}
