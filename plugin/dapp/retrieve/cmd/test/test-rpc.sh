...
function updateConfig() {
    ...
    dplatformos_SignAndSendTx "${unsignedTx}" "0x4257d8692ef7fe13c68b65d6a52f03933db2fa5ce8faf210b5b8b80c721ced01" "${MAIN_HTTP}"
}
...
function token_sendExec() {
    unsignedTx=$(curl -s --data-binary '{"jsonrpc":"2.0","id":2,"method":"DplatformOS.CreateTransaction","params":[{"execer": "'"${token_ame}"'","actionName":"TransferToExec","payload": {"cointoken":"'"$1"'", "amount": "10000000000", "note": "", "to": "'"${retrieve_addr}"'", "execName": "'"${retrieve_name}"'"}}]}' -H 'content-type:text/plain;' ${MAIN_HTTP} | jq -r ".result")
    if [ "${unsignedTx}" == "" ]; then
        echo_rst "token sendExec create tx" 1
        return
    fi

    dplatformos_SignAndSendTx "${unsignedTx}" "$3" "${MAIN_HTTP}"
}
...
retrieve_Backup() {
    ...
    dplatformos_SignAndSendTxWait "$tx" "$retrieve2_key" ${MAIN_HTTP} "$FUNCNAME"
}

retrieve_Prepare() {
    local req='{"method":"retrieve.CreateRawRetrievePrepareTx","params":[{"backupAddr":"'$retrieve1'","defaultAddr":"'$retrieve2'"}]}'
    tx=$(curl -ksd "$req" ${MAIN_HTTP} | jq -r ".result")
    dplatformos_SignAndSendTxWait "$tx" "$retrieve1_key" ${MAIN_HTTP} "$FUNCNAME"
}
...
retrieve_QueryResult() {
    local status=$1
    local req='{"method":"DplatformOS.Query","params":[{"execer":"retrieve","funcName":"GetRetrieveInfo","payload":{"backupAddress":"'$retrieve1'", "defaultAddress":"'$retrieve2'"}}]}'
    dplatformos_Http "$req" ${MAIN_HTTP} '(.result.status == '"$status"')' "$FUNCNAME"
}

retrieve_QueryAssetResult() {
    local status=$1
    local req='{"method":"DplatformOS.Query","params":[{"execer":"retrieve","funcName":"GetRetrieveInfo","payload":{"backupAddress":"'$retrieve1'", "defaultAddress":"'$retrieve2'","assetExec":"token", "assetSymbol":"'"$symbol"'"}}]}'
    dplatformos_Http "$req" ${MAIN_HTTP} '(.result.status == '"$status"')' "$FUNCNAME"
}
...
init() {
    ...
    if [ "$ispara" == true ]; then
        ...
        retrieve_addr=$(curl -ksd '{"method":"DplatformOS.ConvertExectoAddr","params":[{"execname":"user.p.para.retrieve"}]}' ${MAIN_HTTP} | jq -r ".result")
    else
        token_ame="token"
        retrieve_name="retrieve"
        retrieve_addr=$(curl -ksd '{"method":"DplatformOS.ConvertExectoAddr","params":[{"execname":"retrieve"}]}' ${MAIN_HTTP} | jq -r ".result")
    fi
    ...
    dplatformos_ImportPrivkey "${retrieve1_key}" "${retrieve1}" "retrieve1" "${MAIN_HTTP}"
    dplatformos_applyCoins "${retrieve1}" 10000000000 "${MAIN_HTTP}"
    ...
    dplatformos_ImportPrivkey "${retrieve2_key}" "${retrieve2}" "retrieve2" "${MAIN_HTTP}"
    dplatformos_applyCoins "${retrieve2}" 10000000000 "${MAIN_HTTP}"

    if [ "$ispara" == true ]; then
        local main_ip=${MAIN_HTTP//8901/28803}
        dplatformos_applyCoins "${retrieve1}" 1000000000 "${main_ip}"
        dplatformos_applyCoins "${retrieve2}" 1000000000 "${main_ip}"
    fi

    dplatformos_SendToAddress "$retrieve1" "$retrieve_addr" 1000000000 ${MAIN_HTTP}
    dplatformos_SendToAddress "$retrieve2" "$retrieve_addr" 1000000000 ${MAIN_HTTP}
    symbol="RETRIEVE"
...
function main() {
    dplatformos_RpcTestBegin retrieve
    MAIN_HTTP="$1"
    echo "ip=$MAIN_HTTP"

    init
    run_test
    dplatformos_RpcTestRst retrieve "$CASE_ERR"
}
