...
func TransferToken(tokenAddr, fromPrivateKeyStr, toAddr string, amount int64, client *ethclient.Client) (string, error) {
	...
	tx, err := tokenInstance.Transfer(auth, common.HexToAddress(toAddr), big.NewInt(amount))
	if nil != err {
		return "", err
	}
