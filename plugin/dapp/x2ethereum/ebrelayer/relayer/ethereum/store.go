...
var (
	eth2dplatformTxHashPrefix        = "Eth2dplatformTxHash"
	eth2dplatformTxTotalAmount       = []byte("Eth2dplatformTxTotalAmount")
	dplatformToEthTxHashPrefix       = "dplatformToEthTxHash"
...
func calcRelay2DplatformTxhash(txindex int64) []byte {
	return []byte(fmt.Sprintf("%s-%012d", eth2dplatformTxHashPrefix, txindex))
}
...
func (ethRelayer *Relayer4Ethereum) updateTotalTxAmount2dplatform(total int64) error {
	totalTx := &dplatformTypes.Int64{
		Data: atomic.LoadInt64(&ethRelayer.totalTx4Eth2Dplatform),
	}
	//更新成功见证的交易数
	return ethRelayer.db.Set(eth2dplatformTxTotalAmount, dplatformTypes.Encode(totalTx))
}

func (ethRelayer *Relayer4Ethereum) setLastestRelay2DplatformTxhash(txhash string, txIndex int64) error {
	key := calcRelay2DplatformTxhash(txIndex)
...
func (ethRelayer *Relayer4Ethereum) setLogProcHeight(key []byte, height uint64) error {
	...
	return ethRelayer.db.Set(key, dplatformTypes.Encode(data))
}
...
func (ethRelayer *Relayer4Ethereum) getLogProcHeight(key []byte) uint64 {
	...
	err = dplatformTypes.Decode(value, &height)
	if nil != err {
		return 0
	}
	return height.Data
...
func (ethRelayer *Relayer4Ethereum) setBridgeBankProcessedHeight(height uint64, index uint32) {
	bytes := dplatformTypes.Encode(&ebTypes.EventLogIndex{
		Height: height,
		Index:  index})
...
func (ethRelayer *Relayer4Ethereum) getLastBridgeBankProcessedHeight() ebTypes.EventLogIndex {
	...
	_ = dplatformTypes.Decode(data, &logIndex)
	return logIndex
}
