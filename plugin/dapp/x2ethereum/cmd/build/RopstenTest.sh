...
function start_ebrelayerC() {
    ...
    block_wait "${DplatformCli}" $((maturityDegree + 3))
...
function TestDplatformToEthAssets() {
    ...
    result=$(${CLIA} relayer ethereum token4dplatform -s "${tokenSymbol}")
    ...
    # dplatform lock bty
    hash=$(${DplatformCli} send x2ethereum lock -a 5 -t "${tokenSymbol}" -r ${ethReceiverAddr1} -q "${tokenAddrBty}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${DplatformCli}" $((maturityDegree + 3))
    check_tx "${DplatformCli}" "${hash}"
...
function TestETH2DplatformAssets() {
    ...
    hash=$(${DplatformCli} send x2ethereum burn -a 0.1 -t eth -r ${ethReceiverAddr2} -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${DplatformCli}" $((maturityDegree + 3))
    check_tx "${DplatformCli}" "${hash}"
...
function TestETH2DplatformErc20() {
    ...
    # dplatform burn 100
    hash=$(${DplatformCli} send x2ethereum burn -a 100 -t "${tokenSymbol}" -r ${ethReceiverAddr2} -q "${tokenAddr}" -k "${dplatformValidator1}")
    block_wait "${DplatformCli}" $((maturityDegree + 3))
    check_tx "${DplatformCli}" "${hash}"
...
function TestDplatformToEthAssetsKill() {
    ...
    # dplatform lock bty
    hash=$(${DplatformCli} send x2ethereum lock -a 1.41 -t "${tokenSymbol}" -r ${ethReceiverAddr2} -q "${tokenAddrBty}" -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${DplatformCli}" $((maturityDegree + 3))
    check_tx "${DplatformCli}" "${hash}"
    ...
    # eth burn
    result=$(${CLIA} relayer ethereum burn -m 1.41 -k "${ethReceiverAddrKey2}" -r "${dplatformValidator1}" -t "${tokenAddrBty}")
    cli_ret "${result}" "burn"
...
function TestETH2DplatformAssetsKill() {
    ...
    balance=$(${DplatformCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq -r ".balance")
    balance_ret "${result}" "0"

    start_ebrelayerC
    start_ebrelayerD

    result=$(${DplatformCli} x2ethereum balance -s 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv -t eth | jq ".res" | jq ".[]" | jq -r ".balance")
    ...
    hash=$(${DplatformCli} send x2ethereum burn -a 0.133 -t eth -r ${ethReceiverAddr2} -k 12qyocayNF7Lv6C9qW4avxs2E7U41fKSfv)
    block_wait "${DplatformCli}" $((maturityDegree + 3))
    check_tx "${DplatformCli}" "${hash}"
...
function AllRelayerMainTest() {
    set +e
    #    docker cp ${GOPATH}/src/github.com/33cn/plugin/build/ci/x2ethereum build_dplatform_1:/root/x2ethereum
    #    docker cp ${GOPATH}/src/github.com/33cn/plugin/plugin/dapp/x2ethereum/cmd/build/RopstenTest.sh build_dplatform_1:/root/x2ethereum/RopstenTest.sh

    DplatformCli="./../dplatform-cli"
