...
二、创建及参与游戏
1、游戏管理员创建一局竞猜游戏时，可以设定竞猜内容，选项，赌注类型、大小及上限，截止区块高度等，比如竞猜一场足球比赛的结果，每注10DPOM，单次最多可下100注，比赛开始前2小时截止投注(换算成游戏创建后多少个区块)。
...
6、（１）合约先对所有赌注收取一定比例的佣金，比如５‰给开发者地址，５‰给平台地址，（２）合约根据管理员输入的正确结果，对每个投注地址进行输赢判断，并将提取佣金后的剩余所有赌注对所有竞猜正确的地址按各自的投注额占比进行比例分配（比如Ａ选项正确，所有选Ａ的赌注共10000个DPOM，某个地址向Ａ下注100DPOM，则该地址分得1/100）。
